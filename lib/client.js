...
function SttSettingsSection(props) {
  ...
  const ready = snapshot !== void 0 && snapshot.status === "ready";
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "dsh-stt-section", children: [
    ...
    persisted() ? null : /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("p", { className: "dsh-stt-note", children: scope === void 0 ? t("settings.unavailable") : t("settings.readonly") })
  ] });
}
...
function decodeConfig(section) {
  ...
  const raw = section;
  return {
    language: typeof raw.language === "string" && STT_LANGUAGES.includes(raw.language) ? raw.language : DEFAULT_CONFIG.language,
...
function apply(ctx) {
  ...
  ctx.effect(() => {
    ...
    const sync = () => {
      ...
      if (snapshot.value !== void 0) config = snapshot.value;
